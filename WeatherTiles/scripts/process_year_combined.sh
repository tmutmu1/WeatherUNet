...
runprocesshrrrprs() {
    ...
    if [ ! -f  output/combined/hrrr_pressure_velocity/CONUS_$DATE.png  ]; then
       THECMD="./create_rain_combined.sh  $FILE output/combined/hrrr_pressure_velocity/CONUS_$DATE.png hrrr_pressure_velocity" 
       $THECMD
    fi
    if [ ! -f  output/combined/hrrr_pressure/CONUS_$DATE.png  ]; then
       THECMD="./create_rain_combined.sh  $FILE output/combined/hrrr_pressure/CONUS_$DATE.png hrrr_pressure" 
       $THECMD
    fi
    if [ ! -f  output/combined/hrrr_temperature/CONUS_$DATE.png  ]; then
       THECMD="./create_rain_combined.sh  $FILE output/combined/hrrr_temperature/CONUS_$DATE.png hrrr_temperature" 
       $THECMD
    fi 
}
